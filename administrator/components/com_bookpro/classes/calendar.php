...
class PN_CalendarCell {
	...
	public function __construct($y, $m, $d, $in_current_month = true) {
		...
		if ($flight_id) {
			$db=JFactory::getDbo();
			$query=$db->getQuery(true);
			$query->select('*')->from('#__bookpro_flightrate')->where(array('flight_id='.$flight_id,'DATE_FORMAT(date,"%Y-%m-%d")='.$db->quote($dd->format('Y-m-d'))));
			//debug($query->dump());
			$db->setQuery($query);
...
<div data-year="<?php echo $this->cell_year ?>"
	...
	<?php if ($this->rate_price){ 
		...
		?>
	<a class="btn btn-small" target="_blank" href="<?php echo JUri::base(); ?>index.php?option=com_bookpro&view=ratedetail&flight_id=<?php echo $this->flight_id; ?>&date=<?php echo $date; ?>">
		<i class="icon-edit"></i>
	</a>
	<input class="checkboxCell" type="checkbox"  value="<?php echo $date; ?>" name="dates[]" id="<?php echo $date; ?>">
