...
class CurrencyHelper{
	...
	public static function formatprice($value,$config=null){
		...
		$decimalpoint = $config->get('currency_decimalpoint',2);
		$symbol=$config->get('currency_symbol');
		$currency_type = $config->get('currency_display');
	...
	/*public static function formatdistance($value,$config=null){
		if(empty($config))
			$config= JComponentHelper::getParams('com_bookpro');	
	
		$symbol=$config->get('distance_symbol','km');
		return $value.' '.$symbol;
	}*/
	...
	private static function formatNumberWConfig($value,$decimalpoint,$thousand){
		...
		$newval = number_format($value, $decimalpoint, '.',$thousand);
		return $newval;
	}
...
public static function formatpricewithVat($value,$vat = ''){	
	...
	public static function formatPriceM($value,$currency_id){
		...
		if($currency->currency_exchange_rate==1){
			...
			$convetCurr=CurrencyHelper::getMainCurrency();
			$value=CurrencyHelper::convertPrice($value, $convetCurr->id, $currency_id);
		}
