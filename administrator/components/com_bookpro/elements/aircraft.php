...
class JFormFieldAircraft extends JFormFieldList
{
	
	protected function getInput() {
		
		$db = JFactory::getDBO();
		$sql = "SELECT id, title  FROM #__bookpro_aircraft WHERE state=1 ORDER BY id ";
		$db->setQuery($sql);
		$items = $db->loadObjectList();
		
		$options 	= array();
		$options[] 	= JHTML::_('select.option',  '', JText::_('Select Aircraft'), 'id', 'title');
